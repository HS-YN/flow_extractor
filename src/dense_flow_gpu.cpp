...
*/
	...
	//std::cout << "[LOG] Initialization: " << ((float)(clock()-tick)/CLOCKS_PER_SEC) << std::endl;
	//tick = clock();
	...
	while(true){
		...
		if (!initialized){
		    capture_frame = imread(fn[jj++]);
		    //video_stream >> capture_frame;
		    if (jj==count || capture_frame.empty()) return; // read frames until end

            //std::cout << "[LOG] Loading: " << ((float)(clock()-tick)/CLOCKS_PER_SEC) << std::endl;
            //tick = clock();
            ...
            //std::cout << "[LOG] Preprocessing: " << ((float)(clock()-tick)/CLOCKS_PER_SEC) << std::endl;
            //tick = clock();

			cvtColor(prev_image, prev_gray, COLOR_BGR2GRAY);
			initialized = true;
			for(int s = 0; jj <= count && s < step; ++s){
				capture_frame = imread(fn[jj++]);
				//video_stream >> capture_frame;
				cnt++;
				if (jj==count || capture_frame.empty()) return; // read frames until end
			}
            //std::cout << "[LOG] Finished initialization: " << ((float)(clock()-tick)/CLOCKS_PER_SEC) << std::endl;
            //tick = clock();
            ...
            //std::cout << "[LOG] Frame loaded: " << ((float)(clock()-tick)/CLOCKS_PER_SEC) << std::endl;
            //tick = clock();
            ...
            //std::cout << "[LOG] Inference: " << ((float)(clock()-tick)/CLOCKS_PER_SEC) << std::endl;
            //tick = clock();

			//prefetch while gpu is working
			bool hasnext = true && jj <= count;
			for(int s = 0; jj <= count && s < step; ++s){
				capture_frame = imread(fn[jj++]);
				//video_stream >> capture_frame;
				cnt ++;
				hasnext = !(capture_frame.empty() || jj > count);
            ...
            //std::cout << "[LOG] Postprocessing: " << ((float)(clock()-tick)/CLOCKS_PER_SEC) << std::endl;
            //tick = clock();
		}

	}
    //std::cout << "[LOG] Finished. Start saving: " << ((float)(clock()-tick)/CLOCKS_PER_SEC) << std::endl;
